...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Route: /api/regenerate-report - Regenerate insight report with structured JSON (Miratuku News)
      if (path === '/api/regenerate-report') {
        return handleRegenerateReport(request, env, corsHeaders);
      }

      return new Response(JSON.stringify({ error: 'Not found', routes: ['/api/notion', '/api/anthropic', '/api/fetch-url', '/api/foresight', '/api/foresight-builder', '/api/regenerate-report'] }), {
...
// === Regenerate Insight Report (Miratuku News) ===
...
- JSONのみを出力。マークダウンのコードブロックで囲まないこと`;
  ...
  try {
    ...
    return new Response(JSON.stringify({ report: parsed, rawText: answerText, model: data.model, usage: data.usage }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
  } catch (e) {
